...
private func renameWorld(
    objectID: String? = "rename-object",
...
@Test func pathOnlyFolderRenamePreservesFolderIDAndMessages() async throws {
    try await withSyncStore { store, dir in
        let world = renameWorld(objectID: nil, role: .sent)
        world.updateMailbox("Projects/Old") { mailbox in
            mailbox.messages[1] = makePlainMessage(uid: 1, subject: "kept")
            mailbox.uidNext = 2
        }
        let (engine, folder) = try await prepareRenameEngine(store: store, world: world, dir: dir)
        try await waitUntil(timeout: .seconds(5)) {
            try await store.fetchFolderSummary(folder)?.totalCount == 1
        }
        ...
        try await waitUntil(timeout: .seconds(5)) {
            let summary = try await store.fetchFolderSummary(folder)
            let pending = try await store.snapshotFolderRenameQueue()
            return pending.isEmpty && summary?.path == "Projects/New" && summary?.totalCount == 1
        }
        let folders = try await store.fetchFolders(account: sampleConfig().id)
        #expect(folders.contains { $0.id == folder && $0.path == "Projects/New" })
        #expect(!folders.contains { $0.path == "Projects/Old" })
        #expect(folders.filter { $0.path == "Projects/New" }.count == 1)
        await engine.stop()
    }
}
