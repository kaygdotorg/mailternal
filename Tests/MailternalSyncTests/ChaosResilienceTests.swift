...
@Test func diskPolicyHonorsHaltThenHysteresisResume() async throws {
    try await withSyncStore { store, dir in
        let world = ScriptedWorld(
            capabilities: basicCaps(),
            folders: [inboxMailbox()],
            mailboxes: ["INBOX": populatedInbox(uidValidity: 1, count: 8, prefix: "disk")]
        )
        let volume: Int64 = 100 * 1024 * 1024 * 1024
        let reserve = SyncPolicy.reserveBytes(volumeBytes: volume)
        let disk = MutableDisk(freeBytes: 1, volumeBytes: volume)
...
@Test func hostileRoundsLeaveNoStaleGenerationOrSeenOrFTSGhosts() async throws {
    try await withSyncStore { store, dir in
        let world = ScriptedWorld(
            capabilities: qresyncCaps(),
            folders: [inboxMailbox()],
            mailboxes: ["INBOX": populatedInbox(uidValidity: 11, count: 16, prefix: "r1")]
        )
        let (engine, factory) = makeEngine(store: store, world: world, dir: dir, window: 4)
        let events = EventLog()
        let collector = Task {
            for await event in await engine.newMail { events.append(event) }
        }
        ...
        try await runHostileRound(
            store: store,
            world: world,
            engine: engine,
            factory: factory,
            replacementValidity: 21,
            replacementPrefix: "r1b",
            replacementCount: 16
        )
        let afterFirst = events.snapshot().count
        try await runHostileRound(
            store: store,
            world: world,
            engine: engine,
            factory: factory,
            replacementValidity: 31,
            replacementPrefix: "r2",
            replacementCount: 16
        )
        #expect(events.snapshot().count >= afterFirst)

        try await assertStoreInvariants(
            store,
            drain: true,
            expectEmptySeen: true,
            expectNoReplacement: true
        )
        let report = try await store.checkInvariants()
        #expect(report.retiringGenerations == 0)
        collector.cancel()
        await engine.stop()
    }
}

enum ChaosDeltaPath: Sendable {
    case qresync, condstore, basic

    var capabilities: IMAPCapabilities {
        switch self {
        case .qresync: return qresyncCaps()
        case .condstore: return condstoreCaps()
        case .basic: return basicCaps()
        }
    }

    var allowEnable: Bool { self != .condstore }
    var usesVanished: Bool { self == .qresync }
    var expected: DeltaPath {
        switch self {
...
private func runHostileRound(
    store: MailStore,
    world: ScriptedWorld,
    engine: SyncEngine,
    factory: ScriptedFactory,
    replacementValidity: UInt32,
    ...
    world.updateMailbox("INBOX") { live in
        live = populatedInbox(
            uidValidity: replacementValidity,
    ...
    try await waitUntil(timeout: .seconds(8)) {
        ...
        return live.uidValidity == replacementValidity
            && inbox.backfill == .complete
            && inbox.totalCount == Int(replacementCount)
    }

    world.updateMailbox("INBOX") { live in
        let gone = Array(live.messages.keys.sorted().prefix(4))
        for uid in gone { live.messages.removeValue(forKey: uid) }
        live.vanished = gone
    ...
    world.updateMailbox("INBOX") { live in
        let start = live.uidNext
        for offset in 0..<3 {
            let uid = start + UInt32(offset)
            live.messages[uid] = makePlainMessage(
                uid: uid,
                subject: "\(replacementPrefix)-live-\(uid)",
                body: "round live \(uid)"
            )
        }
